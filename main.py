...
async def EloRating(winner, loser, season, winner_score, loser_score):
    ...
    cursor.execute('select count(*) from game_history where (winner_id=%s or loser_id=%s) and season=%s', (winner, season,))
    winner_games = cursor.fetchone()[0]
    cursor.execute('select count(*) from game_history where (winner_id=%s or loser_id=%s) and season=%s', (loser, season,))
