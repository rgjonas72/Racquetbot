...
async def get_history(id1, id2=None):
    id1_name = await get_player_name(id1)
    if id2 is None:
        df = pd.read_sql(f"select player1_id, player1_name, player2_id, player2_name, winner_id, player1_score, player2_score, DATE_FORMAT(game_date,'%d/%m/%Y') as Date from game_history where invalid=0 and (player1_id={id1} or player2_id={id1}) order by game_date desc limit 10", engine)
        title = f'History for {id1_name}'
        user = await client.fetch_user(id1)

    else:
        df = pd.read_sql(f"select player1_id, player1_name, player2_id, player2_name, winner_id, player1_score, player2_score, DATE_FORMAT(game_date,'%d/%m/%Y') as Date from game_history where invalid=0 and ((player1_id={id1} and player2_id={id2}) or (player1_id={id2} and player2_id={id1})) order by game_date desc limit 10", engine)
        id2_name = await get_player_name(id2)
